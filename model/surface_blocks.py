...
def gather(x, idx, method=2):
    ...
    if isinstance(idx, np.ndarray):
        idx = torch.from_numpy(idx).long().to(x.device)
    ...
    if method == 0:
        ...
        for i, ni in enumerate(idx.shape[1:]):
            x = x.unsqueeze(i+1)
            new_s = list(x.size())
            new_s[i+1] = ni
            x = x.expand(new_s)
        n = len(idx.shape)
        for i, di in enumerate(x.shape[n:]):
            idx = idx.unsqueeze(i+n)
            new_s = list(idx.size())
            new_s[i+n] = di
            idx = idx.expand(new_s).to(x.device)
        return x.gather(0, idx.to(torch.long))
    else:
...
class KPConv(nn.Module):
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        if isinstance(s_pts, np.ndarray):
            s_pts = torch.from_numpy(s_pts).float().to(x.device)
        s_pts = torch.cat((s_pts, torch.zeros_like(s_pts[:1, :]) + 1e6), 0)

        # Get neighbor points [n_points, n_neighbors, dim]
        neighbors = s_pts[neighb_inds, :]

        q_pts = torch.tensor(q_pts).to(x.device)
        ...
        neighbors = neighbors.to(deformed_K_points.device)
        ...
        while neighbor_num.dim() < output_features.dim():
            neighbor_num = neighbor_num.unsqueeze(-1)

        if neighbor_num.shape[-1] == 1 and output_features.shape[-1] != 1:
            neighbor_num = neighbor_num.expand(*neighbor_num.shape[:-1], output_features.shape[-1]) 
